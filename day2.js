...
//PART 1 calculate the spreadsheet's checksum. For each row, determine the difference between the largest value and the smallest value; 
//the checksum is the sum of all of these differences.

const arrays = [[1364,461,1438,1456,818,999,105,1065,314,99,1353,148,837,590,404,123], 
...
function solvePart1(arrayOfArrays) {
	let arrayRanges = [];
	for (let i=0; i<arrayOfArrays.length; i++) {
		let currentArray = arrayOfArrays[i];
		arrayRanges.push(Math.max(...currentArray) - Math.min(...currentArray));
	}
	return arrayRanges.reduce(getSum);
}
solvePart1(arrays); //53460
...
function solvePart2(arrayOfArrays) {
	let arrayRanges = [];
	for (let i=0; i<arrayOfArrays.length; i++) {
		let currentArray = arrayOfArrays[i];
		for (let x=0; x<currentArray.length; x++) {
			let startingPoint = x+1;
			for (let s=startingPoint; s<currentArray.length; s++) {
				if (currentArray[x] % currentArray[s] ==0) {
					arrayRanges.push(currentArray[x]/currentArray[s]);
				} else if (currentArray[s] % currentArray[x]==0) {
					arrayRanges.push(currentArray[s]/currentArray[x]);
				}
			}
		}
	}
