//PART 1: To ensure security, a valid passphrase must contain no duplicate words.
//The system's full passphrase list is available as your puzzle input. How many passphrases are valid?

const passcodes = ["vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh",
...
function isDupe(pass1, pass2) {
	if (pass1 == pass2) {
		return true;
	}
}
function solvePuzzle(passcodesArray, thingToCheck) {
	let validPhrases = passcodesArray.length;
	for (let i=0; i< passcodes.length; i++) {
		var currentPhraseArray = passcodes[i].split(" ");;

		oneliner:
		for (let x=0; x<currentPhraseArray.length; x++) {
			let startingPoint = x+1;
			for (let s=startingPoint; s<currentPhraseArray.length; s++) {
				if (thingToCheck(currentPhraseArray[x], currentPhraseArray[s])) {
					validPhrases --;
					break oneliner;
				}
...
solvePuzzle(passcodes, isDupe); //451
...
solvePuzzle(passcodes, isAnagram); //223